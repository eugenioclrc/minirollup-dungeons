...
import signerFuncs from "./signer.js"
import { keccak256 } from "viem";

export default async function run(ROT, pkey) {
    const account = privateKeyToAccount(pkey)
   const { createAccount, updateGame } = signerFuncs(account);
   //window.createAccount = createAccount;
   try {
    // for now we just create the account using the private key storage on the browser
    // on a real scenario we would use metamask to sign the address they will be using 
    // and we got a burner wallet allowed to use send transactions to the minirollup
    await createAccount();
   } catch (e) {
    // nothing
   }
   ...
   let latestState;
   try {
    latestState = await fetch(`http://localhost:3004/game/${account.address}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      });
      latestState = await latestState.json();
      latestState = latestState.game.state;
   } catch (e) {
    // nothing
   }
    ...
    ROT.RNG.setSeed(Number(new Date()));
    ...
    */
    ...
    function runAction(action) {
        ...
        case 'restore-game': {
            try{
                // optimistic use of localStorage, this will be also the same as the on in the mini rollup
        ...
        case 'save-game': {
            ...
            console.log({
                hashedState: keccak256(json),
            });
            json = JSON.parse(json);
            json.tileMap._values = doEncode(tileMap._values);
    ...
    if(latestState != "" && latestState != null){
        deserializeGlobalState(latestState);
    }
}